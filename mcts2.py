...
from network2 import NetworkOutput, Network
import math
import numpy as np
...
class Node(object):
    ...
    def __init__(self, prior: float): 
        ...
        self.value_sum = [0., 0.]   # total value of all visits
    ...
    def value(self) -> float: 
        ...
        return [s/self.visit_count for s in self.value_sum]
    ...
    # - used during MCTS
    def select_child(self, discount: float, min_max_stats: MinMaxStats, alpha):
        _, action, child = max((self._ucb_score(child, discount, min_max_stats, alpha), action, child) for action, child in self.children.items())
        return action, child

    # calculate the UCB score used to select child 
    def _ucb_score(self, child, discount: float, min_max_stats: MinMaxStats, alpha) -> float:
        ...
        if child.visit_count > 0 and self.visit_count > 3:
            val = child.value()
            value_score = [min_max_stats[0].normalize(child.reward + discount[0] * val[0]),
                              min_max_stats[1].normalize(child.reward + discount[1] * val[1])]
        else:
            value_score = [0.,0.]
            
        #alpha = 1. if self.visit_count > 20 else 0.
        #vals = np.random.select(value_score,1,p=[alpha,1-alpha])
        return prior_score + value_score[0]*alpha + value_score[1]*(1-alpha)
...
def backpropagate(search_path: List[Node], value: float, to_play: Player, discount: float, min_max_stats: MinMaxStats): 
    ...
    for node in reversed(search_path):
        node.value_sum[0] += value #if node.to_play == to_play else -value 
        node.value_sum[1] += value2 #if node.to_play == to_play else -value 
        node.visit_count += 1
        
        min_max_stats[0].update(node.value()[0])
        min_max_stats[1].update(node.value()[1])
        
        value = node.reward + discount[0] * value
        value2 = node.reward + discount[1] * value2


# the MCTS process
def run_mcts(config: MCTSConfig, root: Node, action_history: ActionHistory, network: Network, alpha):
    
    min_max_stats = [MinMaxStats(config.known_bounds) for _ in root.value_sum]
    ...
    for _ in range(config.num_simulations): 
        ...
        while node.expanded():
            action, node = node.select_child(config.discount, min_max_stats, alpha) 
