...
class Network():
    ...
    def __init__(self):
        ...
        x9 =layers.Dense(M)(x4)
        x10 =layers.Activation('relu')(x9)
        value2 = layers.Dense(600)(x10)
        self.prediction = Model(hstate, [policy, value, value2])
    ...
    def initial_inference(self, image) -> NetworkOutput: 
        ...
        value = [decategorise(tf.nn.softmax(v)) for v in value]
        reward = decategorise(tf.nn.softmax(reward))
        return NetworkOutput([v[0].numpy() for v in value],
                            reward[0].numpy(),  
    ...
    def _initial_inference(self, image) -> Tuple: 
        ...
        # hidden (None, N)
        policy_logits, value, value2 = self.prediction(hidden)
        ...
        return ([value, value2], tf.zeros(value.shape), policy_logits, hidden)
    ...
    def _recurrent_inference(self, hidden_state, action) -> Tuple: 
        ...
        #policy_logits = tf.divide(policy_logits, tf.reduce_sum(policy_logits,axis = 1, keepdims= True))
        return ([value, value2], reward, policy_logits, hidden)        
    ...
    def update_weights(self, batch, weight_decay: float, hidden_state_dampen: float = 0.5):
        ...
        with tf.GradientTape() as g:
            ...
            for prediction, target in zip(predictions, targets): 
                ...
                value_loss = scalar_loss(prediction = value[0], target =  target_value[:,0])
                ...
                value_loss2 = scalar_loss(prediction = value[1], target =  target_value[:,1])
                #vLoss += tf.reduce_mean(scale_gradient(value_loss, gradient_scale))
                v2Loss += tf.reduce_mean(tf.boolean_mask(value_loss2*gradient_scale, target_mask))
            ...
            # combine the losses     
            loss = (Network.grad_scale[0]*(vLoss+v2Loss) + Network.grad_scale[1]*rLoss + Network.grad_scale[2]*pLoss)
        ...
        return (vLoss.numpy(), v2Loss.numpy(), rLoss.numpy(), pLoss.numpy())

    def save_to_file(self):
        self.representation.save('repmodel.h5')
        self.dynamics.save('dynmodel.h5')
    ...
    def load_from_file(self):
        ...
        self.optimiser.learning_rate(learning_rate, 0.9)
        infile.close()
